class_name SnoreGeyser
extends Area3D
## SnoreGeyser — Bramble's snore updraft (PITCH.md: "ride snore-geysers up
## through the fur"). On a repeating cycle it goes active for a short window;
## while active, any overlapping PlayerBody's vertical velocity is eased
## up toward `vent_speed` — never snapped, never allowed to push velocity.y
## down. Builds its own translucent column visual + detection collider from
## `radius`/`height` so bramble.gd only has to place and phase-offset it.
...
func _build_geometry() -> void:
	...
	mat.albedo_color = Color(0.961, 0.949, 0.910, 0.45)
...
func _set_active(is_active: bool) -> void:
	...
	if is_active:
		AudioManager.play_sfx("snore_geyser")
		activated.emit()
	else:
		deactivated.emit()
